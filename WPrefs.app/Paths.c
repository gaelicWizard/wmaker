...
static void showData(_Panel * panel)
{
	...
	if (!array || !WMIsPLArray(array)) {
		...
		addPathToList(panel->icoL, -1, "~"PIXMAP_SUBDIR);
		addPathToList(panel->icoL, -1, GSUSER_DIR""USERDATA_SUBDIR"/Icons");
		addPathToList(panel->icoL, -1, "/usr/include/X11/pixmaps");
		addPathToList(panel->icoL, -1, PKGDATADIR"/Icons");
		addPathToList(panel->icoL, -1, PKGDATADIR"/Pixmaps");
	...
	if (!array || !WMIsPLArray(array)) {
		...
		addPathToList(panel->pixL, -1, "~"PIXMAP_SUBDIR);
		addPathToList(panel->pixL, -1, GSUSER_DIR""USERDATA_SUBDIR"/Pixmaps");
		addPathToList(panel->pixL, -1, PKGDATADIR"/Pixmaps");
	} else {
		for (i = 0; i < WMGetPropListItemCount(array); i++) {
			val = WMGetFromPLArray(array, i);
